...
def train(data):
    ...
    train_data = {'image': x_train, 'quadrant': y_train}
    train_df_augmented = pd.concat([pd.DataFrame(train_data)] * 10, ignore_index=True)

    # Ensure 'image' and 'quadrant' are 1-dimensional arrays
    x_train_augmented = np.stack(train_df_augmented['image'])
    y_train_augmented = train_df_augmented['quadrant'].to_numpy()
    y_train_augmented = to_categorical(y_train_augmented, 9)

    # Data augmentation for test data
    test_data = {'image': x_test, 'quadrant': y_test}
    test_df_augmented = pd.concat([pd.DataFrame(test_data)] * 5, ignore_index=True)

    # Ensure 'image' and 'quadrant' are 1-dimensional arrays
    x_test_augmented = np.stack(test_df_augmented['image'])
    y_test_augmented = test_df_augmented['quadrant'].to_numpy()
    y_test_augmented = to_categorical(y_test_augmented, 9)

    # Applying data augmentation using ImageDataGenerator
    datagen = ImageDataGenerator(...)

    datagen_train = datagen.flow(x_train_augmented, y_train_augmented, batch_size=10)
    datagen_val = datagen.flow(x_test_augmented, y_test_augmented, batch_size=10)
